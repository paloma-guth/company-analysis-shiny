...
ui <- fluidPage(
  titlePanel("Company Financial Metrics Overview"),
  selectInput("company_choice", "Select a Company:", choices = unique(summarized_data$company)),
...
server <- function(input, output) {
  output$company_plot <- renderPlot({
    generate_plot(summarized_data, input$company_choice)
  })
}
...
# List of all balance sheet datasets with company identifiers
...
# Combine all balance sheet datasets into one
...
ui <- fluidPage(
  titlePanel("Company Balance Sheet Overview"),
  selectInput("company_choice", "Select a Company:", choices = unique(summarized_balance_data$company)),
  plotOutput("balance_plot")
)

server <- function(input, output) {
  output$balance_plot <- renderPlot({
    generate_balance_plot(summarized_balance_data, input$company_choice)
  })
}
